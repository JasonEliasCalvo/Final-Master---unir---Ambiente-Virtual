...
namespace Convai.Scripts.Runtime.Core
{
    ...
    public class ConvaiNPC : MonoBehaviour
    {
        ...
        {
            Debug.Log($"SetInputEnabled: {enabled}");
            FindFirstObjectByType<ChatBoxUI>().InputField.SetActive(enabled);
            var playerInteractionManager = GetComponent<ConvaiPlayerInteractionManager>();

            if (playerInteractionManager == null) return;

            if (!enabled)
                playerInteractionManager._inputManager.talkKeyInteract -= playerInteractionManager.HandleVoiceInput;
