...
namespace Convai.Scripts.Runtime.Features
{
    ...
    public class ConvaiActionsHandler : MonoBehaviour
    {
        ...
        private int walkingLayerIndex = 1; // índice de tu capa "Walking"
        ...
        private void Awake()
        {
            ...
            if (_interactablesData != null)
            {
                ...
                // Iterate through each hall in global action settings and add them to the action configuration
                foreach (ConvaiInteractablesData.Hall eachHall in _interactablesData.Halls)
                {
                    foreach (var obj in eachHall.Objects)
                    {
                        ActionConfig.Types.Object rpcObject = new()
                        {
                            name = obj.Name,
                            description = $"Elemento que hace parte del conjunto {eachHall.Name}: {obj.Description}. Descripción del conjunto: {eachHall.Description}"
                        };

                        ActionConfig.objects.Add(rpcObject);
                    }

                    ActionConfig.Types.Object hallObject = new()
                    {
                        name = eachHall.Name,
                        description = $"Salón: {eachHall.Description}. Contiene {eachHall.Objects.Count} elemento(s)."
                    };
                    ActionConfig.objects.Add(hallObject);
                }
        ...
        public void RefreshObjectsFromConvaiData()
        {
            if (_interactablesData == null) return;

            ActionConfig.objects.Clear();

            foreach (var eachObject in _interactablesData.Objects)
            {
                ActionConfig.Types.Object rpcObject = new()
                {
                    name = eachObject.Name,
            ...
            foreach (var eachHall in _interactablesData.Halls)
            {
                foreach (var obj in eachHall.Objects)
                {
                    ActionConfig.Types.Object rpcObject = new()
                    {
                        name = obj.Name,
                        description = $"Ubicado en el salón {eachHall.Name}: {obj.Description}. Descripción del salón: {eachHall.Description}"
                    };
                    ActionConfig.objects.Add(rpcObject);
                }
                ...
                {
                    name = eachHall.Name,
                    description = $"Salón: {eachHall.Description}. Contiene {eachHall.Objects.Count} objeto(s)."
                };
                ActionConfig.objects.Add(hallObject);
            }

            var grpc = FindAnyObjectByType<ConvaiGRPCWebAPI>();
            if (grpc != null)
            {
                grpc.UpdateActionConfig(ActionConfig);
                Debug.Log("[Convai] ActionConfig actualizado y enviado a WebAPI");
            }
        }
        ...
        private GameObject FindTargetObject(string[] objectPart)
        {
            ...
            // Try to find a matching Hall
            ConvaiInteractablesData.Hall hall = _interactablesData.Halls
                .OrderBy(o => LevenshteinDistance(o.Name.ToLower(), targetName.ToLower()))
                .FirstOrDefault();

            if (hall != null && LevenshteinDistance(hall.Name.ToLower(), targetName.ToLower()) <= 2)
                return hall.gameObject;
        ...
        public IEnumerator Follow(Transform target, TourGuideController guide, float stoppingDistance = 5f)
        {
            ...
            while (_following)
            {
                float distance = Vector3.Distance(_currentNPC.transform.position, target.position);
                ...
                else
                {
                    ...
                    FinishMovement(animator, target.gameObject);
                    guide.FollowPlayer(false);
                    target = null;
        ...
        private void SetupAnimationAndNavigation(Animator animator, NavMeshAgent navMeshAgent)
        {
            ...
            {
                animator.CrossFade(Animator.StringToHash("Walking"), 0.003f);
            }

            StartCoroutine(LerpLayerWeight(animator, walkingLayerIndex, 1f, 0.3f));
        ...
        private void FinishMovement(Animator animator, GameObject target)
        {
            ...
            StartCoroutine(LerpLayerWeight(animator, walkingLayerIndex, 0f, 0.3f));

            if (Camera.main != null) StartCoroutine(RotateTowardsCamera());
            animator.applyRootMotion = false;
        ...
        private IEnumerator LerpLayerWeight(Animator animator, int layer, float targetWeight, float duration)
        {
            ...
            while (time < duration)
            {
                ...
                animator.SetLayerWeight(layer, Mathf.Lerp(startWeight, targetWeight, t));
                yield return null;
            }
